...
//	auto start = chrono::steady_clock::now();
	...
	for (x = 0; x < 30; x++) {									// for each position
		for (y = 0; y < 69; y++) {
			...
			hist_k_nn(roi, img_data, cond_idx, cond_k, hist_k, hist_idx);
			best_idx = best_match(roi, img_data, hist_idx, hist_k);

			v_unit[y] = img_data[best_idx];						// put in current column
		}
...
//	cout << chrono::duration<double>(diff).count() << '\n';
	...
	cout << "saving collage...\n......\n";
	collage_name = "..\\..\\..\\Resources\\30_69_v4_k" + to_string(cond_k) + 
